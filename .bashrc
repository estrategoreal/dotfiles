...
if is_darwin ; then
  ...
  alias mint='/Applications/VirtualBox.app/Contents/MacOS/VBoxManage startvm Mint14 --type gui'
...
function ide() {
  if [[ $# -ge 1 ]] ; then
    gvim +"set columns=177" +"VimFiler -buffer-name=explorer -simple -toggle ~" +"tabedit $1" +TlistOpen +"VimFilerExplorer -winwidth=46" +"wincmd l"
  else
    gvim +"set columns=177" +"VimFiler -buffer-name=explorer -simple -toggle ~" +tabnew +TlistOpen +"wincmd h" +"VimFilerExplorer -winwidth=46"
  fi
}
...
if is_darwin ; then
  ...
  export PATH=$PATH:/usr/local/share/vim:/usr/local/share/git-svn-clone-externals:/usr/local/share/packer
